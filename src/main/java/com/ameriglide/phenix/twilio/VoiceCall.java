  ...
  protected void get(final HttpServletRequest request, final HttpServletResponse response) throws Exception {
    ...
    if (caller.isAgent()) {
      ...
      if (called.isAgent()) {
        ...
        twiml = new VoiceResponse.Builder()
          .dial(new Dial.Builder().number(TwiMLServlet.buildNumber(called)).callerId(vCid.getPhoneNumber()).build())
          .build();
      }
    } else {
      ...
      if (called.isAgent()) {
        ...
        twiml = null;
        log.info(() -> "%s is a new call to %s in service  of task %s".formatted(callSid, called.agent().getFullName(),
          taskCall.sid));
      } else {
        ...
        if (vCid==null || vCid.isIvr()) {
          ...
          log.info(
            () -> "%s is a new queue call %s->%s (%s)".formatted(callSid, caller, called, vCid.getQueue().getName()));
